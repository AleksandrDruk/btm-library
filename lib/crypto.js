...
const AFFILIATE_SITE_CONTEXT = 'btm-affiliate-site-v1';
...
export function normalizeAffiliateSiteId(value) {
  const siteId = String(value || '').trim().toLowerCase();
  if (!/^[a-z0-9][a-z0-9.-]{2,99}$/.test(siteId) || siteId.includes('..')) {
    throw new Error('Site id must contain 3-100 lowercase letters, digits, dots or hyphens.');
  }
  return siteId;
}

async function importAffiliateHmacKey(secret, usage) {
  const bytes = base64ToBytes(secret);
  if (bytes.length < 32 || bytes.length > 128) {
    throw new Error('Affiliate catalog secret must contain 32-128 bytes.');
  }
  return crypto.subtle.importKey('raw', bytes, { name: 'HMAC', hash: 'SHA-256' }, false, usage);
}

export async function deriveAffiliateSiteSecret(masterSecret, siteId) {
  const normalizedSiteId = normalizeAffiliateSiteId(siteId);
  const masterKey = await importAffiliateHmacKey(masterSecret, ['sign']);
  const derived = await crypto.subtle.sign(
    'HMAC',
    masterKey,
    encoder.encode(`${AFFILIATE_SITE_CONTEXT}\n${normalizedSiteId}`),
  );
  return bytesToBase64(new Uint8Array(derived));
}

export function affiliateSiteCanonicalRequest(siteId, timestamp, nonce) {
  const normalizedSiteId = normalizeAffiliateSiteId(siteId);
  const numericTimestamp = Number(timestamp);
  if (!Number.isInteger(numericTimestamp) || numericTimestamp < 1) {
    throw new Error('Affiliate catalog timestamp is invalid.');
  }
  if (typeof nonce !== 'string' || !/^[A-Za-z0-9_-]{22,64}$/.test(nonce)) {
    throw new Error('Affiliate catalog nonce is invalid.');
  }

  return `POST\n/affiliate-catalog/read\n${normalizedSiteId}\n${numericTimestamp}\n${nonce}`;
}

export async function createAffiliateSiteSignature(siteSecret, siteId, timestamp, nonce) {
  const key = await importAffiliateHmacKey(siteSecret, ['sign']);
  const signature = await crypto.subtle.sign(
    'HMAC',
    key,
    encoder.encode(affiliateSiteCanonicalRequest(siteId, timestamp, nonce)),
  );
  return bytesToBase64Url(new Uint8Array(signature));
}

export async function verifyAffiliateSiteSignature(signature, siteSecret, siteId, timestamp, nonce) {
  if (typeof signature !== 'string' || !/^[A-Za-z0-9_-]{43}$/.test(signature)) {
    return false;
  }

  try {
    const key = await importAffiliateHmacKey(siteSecret, ['verify']);
