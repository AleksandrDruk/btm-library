...
export const ALLOWED_AFFILIATE_PLACEHOLDERS = new Set([
  'YOURCLICKID',
  'YOURSECONDPARAM',
  'clickid',
  'sub1',
...
function normalizeGeo(value) {
  ...
  if (normalized !== 'GLOBAL' && !ISO_GEOS.has(normalized)) {
    fail('invalid_geo', 'GEO должен быть ISO-кодом из двух букв или GLOBAL.');
  }
  return normalized;
}

function validateDestinationPlaceholders(value) {
  const rawPattern = /\{([A-Za-z0-9_]{1,64})\}/g;
  const encodedPattern = /%7B([A-Za-z0-9_]{1,64})%7D/gi;
  const rawTokens = Array.from(value.matchAll(rawPattern), (match) => match[1]);
  const encodedTokens = Array.from(value.matchAll(encodedPattern), (match) => match[1]);
  const withoutRawTokens = value.replace(rawPattern, '');
  const withoutEncodedTokens = value.replace(encodedPattern, '');

  if (/[{}]/.test(withoutRawTokens) || /%7B|%7D/i.test(withoutEncodedTokens)) {
    fail('invalid_destination_placeholder', 'Affiliate URL содержит несбалансированный tracking placeholder.');
  }

  for (const token of [...rawTokens, ...encodedTokens]) {
    if (!ALLOWED_AFFILIATE_PLACEHOLDERS.has(token)) {
      fail('invalid_destination_placeholder', 'Affiliate URL содержит неподдерживаемый tracking placeholder.');
    }
  }
}
...
export function normalizeDestinationUrl(value) {
  ...
  validateDestinationPlaceholders(normalized);
...
export function validateAffiliateCatalogTransition(baseValue, candidateValue, options = {}) {
  ...
  const baseBrandIds = new Map(base.items.map((item) => [affiliateBrandKey(item.brand), item.id]));
  for (const item of candidate.items) {
    const previousBrandId = baseBrandIds.get(affiliateBrandKey(item.brand));
    if (previousBrandId && previousBrandId !== item.id) {
      fail('brand_id_transition', `Бренд ${item.brand} должен сохранить id ${previousBrandId}.`);
    }
    ...
    const previousLinkIds = new Map(previous.links.map((link) => [
      `${link.geo}\n${destinationKey(link.destination_url)}`,
      link.id,
    ]));
    for (const link of item.links) {
      const previousLinkId = previousLinkIds.get(`${link.geo}\n${destinationKey(link.destination_url)}`);
      if (previousLinkId && previousLinkId !== link.id) {
        fail('link_id_transition', `Позиция ${item.id}: существующая ссылка должна сохранить id ${previousLinkId}.`);
      }
    }
