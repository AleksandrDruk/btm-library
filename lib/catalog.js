...
export const LEGACY_CATALOG_SCHEMA_VERSION = 1;
...
const ITEM_KEYS = [...LEGACY_ITEM_KEYS, 'sha256'];
...
function normalizeCatalogItem(item, index, schemaVersion) {
  ...
  assertExactKeys(
    item,
    schemaVersion === CATALOG_SCHEMA_VERSION ? ITEM_KEYS : LEGACY_ITEM_KEYS,
    `Позиция ${index + 1}`,
  );
  ...
  if (schemaVersion === CATALOG_SCHEMA_VERSION) {
    if (typeof item.sha256 !== 'string' || !SHA256_PATTERN.test(item.sha256)) {
      fail('invalid_sha256', `Позиция ${index + 1}: sha256 должен быть lowercase SHA-256.`);
    }
...
export function validateCatalog(value) {
  ...
  if (![LEGACY_CATALOG_SCHEMA_VERSION, CATALOG_SCHEMA_VERSION].includes(value.schema_version)) {
    fail(
      'unsupported_schema',
      `Поддерживаются schema_version ${LEGACY_CATALOG_SCHEMA_VERSION} и ${CATALOG_SCHEMA_VERSION}.`,
    );
  }
  ...
  const items = value.items.map((item, index) => normalizeCatalogItem(item, index, value.schema_version));
  const ids = new Set();
  const paths = new Set();
  ...
  for (const item of items) {
    ...
    if (item.sha256) {
      const duplicate = imageDigests.get(item.sha256);
      if (duplicate) {
        fail('duplicate_image_content', `Изображение ${item.path} дублирует ${duplicate.path}.`);
      }
      imageDigests.set(item.sha256, item);
    }
    ids.add(item.id);
    paths.add(item.path);
...
export function buildCatalogUpdate(currentCatalog, entries) {
  ...
  const usedImageDigests = new Map(
    items
  ...
  entries.forEach((entry, fileIndex) => {
    ...
    const imageDigest = entry.image.sha256;
    if (imageDigest !== undefined && (typeof imageDigest !== 'string' || !SHA256_PATTERN.test(imageDigest))) {
      fail('invalid_sha256', `Файл ${fileIndex + 1}: некорректный SHA-256.`);
    }
    if (catalog.schema_version === CATALOG_SCHEMA_VERSION && !imageDigest) {
      fail('invalid_sha256', `Файл ${fileIndex + 1}: отсутствует корректный SHA-256.`);
    }
    const duplicatePath = imageDigest ? usedImageDigests.get(imageDigest) : '';
    if (duplicatePath) {
      fail('duplicate_image_content', `Файл ${fileIndex + 1}: это изображение уже существует (${duplicatePath}).`);
    }
    let id;
    let version;
    let itemIndex = -1;
    ...
    usedPaths.add(path);
    if (imageDigest) {
      usedImageDigests.set(imageDigest, path);
    }
